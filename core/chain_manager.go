...
func CalcGasLimit(parent, block *types.Block) *big.Int {
	...
	return thelutil.BigMax(min, result)
}
...
func (bc *ChainManager) NewBlock(coinbase []byte) *types.Block {
	...
	if parent != nil {
		...
		header.Number = new(big.Int).Add(parent.Header().Number, thelutil.Big1)
...
func (self *ChainManager) Export() []byte {
	...
	return thelutil.Encode(blocks)
}

func (bc *ChainManager) insert(block *types.Block) {
	encodedBlock := thelutil.Encode(block)
...
func (bc *ChainManager) write(block *types.Block) {
	...
	encodedBlock := thelutil.Encode(block.RlpDataForStorage())
...
func (self *ChainManager) GetBlockHashesFromHash(hash []byte, max uint64) (chain [][]byte) {
	...
	for i := uint64(0); i < max; i++ {
		...
		if block.Header().Number.Cmp(thelutil.Big0) <= 0 {
			break
		}
	}
...
func (self *ChainManager) InsertChain(chain types.Blocks) error {
	for _, block := range chain {
		...
		self.mu.Lock()
		{
			...
			if td.Cmp(self.td) > 0 {
				if block.Header().Number.Cmp(new(big.Int).Add(cblock.Header().Number, thelutil.Big1)) < 0 {
