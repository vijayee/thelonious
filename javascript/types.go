...
func (self *JSStateObject) EachStorage(call otto.FunctionCall) otto.Value {
	...
	for it.Next() {
		cb.Call(self.eth.toVal(self), self.eth.toVal(thelutil.Bytes2Hex(it.Key)), self.eth.toVal(thelutil.Bytes2Hex(it.Value)))
	}
...
func NewJSMessage(message *state.Message) JSMessage {
	return JSMessage{
		To:        thelutil.Bytes2Hex(message.To),
		From:      thelutil.Bytes2Hex(message.From),
		Input:     thelutil.Bytes2Hex(message.Input),
		Output:    thelutil.Bytes2Hex(message.Output),
		Path:      message.Path,
		Origin:    thelutil.Bytes2Hex(message.Origin),
		Timestamp: int32(message.Timestamp),
		Coinbase:  thelutil.Bytes2Hex(message.Origin),
		Block:     thelutil.Bytes2Hex(message.Block),
...
func (self *JSEthereum) GetStateObject(addr string) otto.Value {
	return self.toVal(&JSStateObject{xeth.NewJSObject(self.JSXEth.World().SafeGet(thelutil.Hex2Bytes(addr))), self})
}
