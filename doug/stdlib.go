...
func VariName(name string) []byte {
	h := crypto.Sha3(thelutil.PackTxDataArgs2(name))
...
// (+ @@variname 1)
func GetSingle(addr []byte, name string, state *monkstate.StateDB) []byte {
	obj := state.GetStateObject(addr)
	base := VariName(name)
	base[31] = byte(StdVarSize + 1)
	return (obj.GetStorage(thelutil.BigD(base))).Bytes()
}

// Return an element from an array type
func GetArrayElement(addr []byte, name string, index int, state *monkstate.StateDB) []byte {
	return GetKeyedArrayElement(addr, name, "0x0", index, state)
}

// Return the entire array
func GetArray(addr []byte, name string, state *monkstate.StateDB) [][]byte {
	return GetKeyedArray(addr, name, "0x0", state)
}

// key must come as hex!!!!!!
// Return an element from keyed array type
func GetKeyedArrayElement(addr []byte, name, key string, index int, state *monkstate.StateDB) []byte {
	...
	elementSizeBytes := (obj.GetStorage(thelutil.BigD(sizeLocator))).Bytes()
	elementSize := thelutil.BigD(elementSizeBytes).Uint64()
	...
	keyBytes := thelutil.PackTxDataArgs2("0x" + key)
	keyBytesShift := append(keyBytes[3:], []byte{1, 0, 0}...)
	slotBig := bigBase.Add(thelutil.BigD(base), thelutil.BigD(keyBytesShift))
	...
	rowStorage := (obj.GetStorage(thelutil.BigD(row))).Bytes()
	rowStorageBig := thelutil.BigD(rowStorage)

	elSizeBig := thelutil.BigD(elementSizeBytes)
...
func GetKeyedArray(addr []byte, name, key string, state *monkstate.StateDB) [][]byte {
	return nil
}

// Return an element from a linked list
func GetLinkedListElement(addr []byte, name, key string, state *monkstate.StateDB) []byte {
	...
	keyBytes := thelutil.PackTxDataArgs2(key)
...
func traverseLink(addr []byte, name, key string, state *monkstate.StateDB, dir int) ([]byte, []byte) {
	bigBase := big.NewInt(0)
	...
	if slotNext.IsNil() {
		slotNextLoc = thelutil.BigD(append(base[:len(base)-1], byte(StdVarSize+1)))
		slotNext = obj.GetStorage(slotNextLoc)
	}
...
func GetNextLinkedListElement(addr []byte, name, key string, state *monkstate.StateDB) ([]byte, []byte) {
	return traverseLink(addr, name, key, state, 2)
}

func GetPrevLinkedListElement(addr []byte, name, key string, state *monkstate.StateDB) ([]byte, []byte) {
	return traverseLink(addr, name, key, state, 1)
}

func GetLinkedListLength(addr []byte, name string, state *monkstate.StateDB) int {
	obj := state.GetStateObject(addr)
	base := VariName(name)
...
func getLinkedListLength(obj *monkstate.StateObject, base []byte) int {
	...
	nEl := obj.GetStorage(thelutil.BigD(nEleLoc))
	return int(nEl.Uint())
}

func GetLinkedListHead(addr []byte, name string, state *monkstate.StateDB) ([]byte, []byte) {
	obj := state.GetStateObject(addr)
	base := VariName(name)
...
func getLinkedListHead(obj *monkstate.StateObject, base []byte) ([]byte, []byte) {
	...
	headLoc := obj.GetStorage(thelutil.BigD(headLocLoc))
	headKey := headLoc.Bytes()[9:29]
	head := obj.GetStorage(headLoc.BigInt())
	return headKey, head.Bytes()
}

func GetLinkedList(addr []byte, name string, state *monkstate.StateDB) *ring.Ring {
	bigBase := big.NewInt(0)
	...
	nextLocLoc := thelutil.BigD(append(base[:len(base)-1], byte(StdVarSize+1)))
