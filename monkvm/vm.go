...
type Environment interface {
	...
	BlockHash() []byte
	Doug() []byte
	DougValidate(addr []byte, role string, state *monkstate.StateDB) error
}

type Object interface {
	GetStorage(key *big.Int) *thelutil.Value
	SetStorage(key *big.Int, value *thelutil.Value)
}
...
func calcMemSize(off, l *big.Int) *big.Int {
	if l.Cmp(thelutil.Big0) == 0 {
		return thelutil.Big0
	}
		...
		*self.callStack = (*self.callStack)[:len(*self.callStack)-1]
		...
		if self.Dump {
			...
			for i := 0; i < mem.Len()/32; i++ {
				fmt.Println("\t", i, thelutil.Bytes2Hex(mem.Get(int64(i*32+137), 32)))
			}
		}
	}()
	...
	for {
		...
		if self.logTy == LogTyDiff {
			...
			case STOP, RETURN, SUICIDE:
				closure.object.EachStorage(func(key string, value *thelutil.Value) {
					value.Decode()
		...
		case SSTORE:
			...
			if val.BigInt().Cmp(thelutil.Big0) == 0 && len(y.Bytes()) > 0 {
				mult = thelutil.Big2
			} else if val.BigInt().Cmp(thelutil.Big0) != 0 && len(y.Bytes()) == 0 {
				mult = thelutil.Big0
			} else {
				mult = thelutil.Big1
			}
		...
		case RLPDECODE:
			...
			decoded, _ := thelutil.Decode(rawrlp, 0)
		...
		case RLPDECODE:
			...
			mem.Set(start, 32, thelutil.LeftPadBytes([]byte{byte(N)}, 32))
			...
			for i, dd := range d {
				...
				mem.Set(start+32*(2*i64+1), 32, thelutil.LeftPadBytes(pointer.Bytes(), 32))
				...
				mem.Set(start+32*(2*i64+2), 32, thelutil.LeftPadBytes(length, 32))

				// set the actual chunk
				b = thelutil.LeftPadBytes(b, 32)
		...
		case RLPENCODE:
			...
			for i := 0; int64(i) < N.Int64(); i++ {
				...
				pointerInt := thelutil.BigD(pointer).Int64()
				b := mem.Get(pointerInt, 32)
				if len(b) > 0 {
					b = b[int64(len(b))-thelutil.BigD(length).Int64():]
				}

				data = append(data, b)
			}

			rlpdata := thelutil.Encode(data)
			...
			// 0x30 range
		case ADDRESS:
			stack.Push(thelutil.BigD(closure.Address()))
		...
		case PREVHASH:
			...
			stack.Push(thelutil.BigD(prevHash))
		...
		case DUP1, DUP2, DUP3, DUP4, DUP5, DUP6, DUP7, DUP8, DUP9, DUP10, DUP11, DUP12, DUP13, DUP14, DUP15, DUP16:
			...
			if OpCode(closure.Get(new(big.Int).Add(pc, thelutil.Big1)).Uint()) == POP && OpCode(closure.Get(new(big.Int).Add(pc, big.NewInt(2))).Uint()) == POP {
		...
		case MSTORE: // Store the value at stack top-1 in to memory at location stack top
			...
			mem.Set(mStart.Int64(), 32, thelutil.BigToBytes(val, 256))
		...
		case SSTORE:
			require(2)
			val, loc := stack.Popn()
			closure.SetStorage(loc, thelutil.NewValue(val))
		...
		case CREATE:
			...
			for i := uint64(0); self.env.State().GetStateObject(addr) != nil; i++ {
				...
				crypto.CreateAddress(closure.Address(), closure.object.Nonce+i)
			}
...
func (self *Message) Exec(codeAddr []byte, caller ClosureRef) (ret []byte, err error) {
	...
	if object.Balance().Cmp(self.value) < 0 {
		...
		baddr := thelutil.BigD(self.address).Bytes()
