...
type ClosureRef interface {
	...
	GetStorage(*big.Int) *thelutil.Value
	SetStorage(*big.Int, *thelutil.Value)
}
...
func (c *Closure) GetStorage(x *big.Int) *thelutil.Value {
	m := c.object.GetStorage(x)
	if m == nil {
		return thelutil.EmptyValue()
	}

	return m
}

func (c *Closure) Get(x *big.Int) *thelutil.Value {
	return c.Gets(x, big.NewInt(1))
}

func (c *Closure) Gets(x, y *big.Int) *thelutil.Value {
	...
	return thelutil.NewValue(partial)
}

func (c *Closure) SetStorage(x *big.Int, val *thelutil.Value) {
