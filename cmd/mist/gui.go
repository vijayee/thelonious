...
import (
	...
	"github.com/eris-ltd/new-thelonious/ui/qt/qwhisper"
...
func NewWindow(ethereum *eth.Ethereum, config *thelutil.ConfigManager, clientIdentity *p2p.SimpleClientIdentity, session string, logLevel int) *Gui {
	db, err := theldb.NewLDBDatabase("tx_database")
	if err != nil {
		panic(err)
	}
	...
	data, _ := thelutil.ReadAllFile(path.Join(thelutil.Config.ExecPath, "plugins.json"))
...
func (gui *Gui) loadAddressBook() {
	...
	if nameReg != nil {
		...
		for it.Next() {
			if it.Key[0] != 0 {
				view.Call("addAddress", struct{ Name, Address string }{string(it.Key), thelutil.Bytes2Hex(it.Value)})
			}
	...
	if mergeMining != nil {
		...
		for it.Next() {
			view.Call("addMergedMiningOption", struct {
				...
				Name, Address string
				Id, ItemId    int
			}{false, string(it.Key), thelutil.Bytes2Hex(it.Value), 0, i})
...
func (gui *Gui) insertTransaction(window string, tx *types.Transaction) {
	...
	if send.Len() != 0 {
		...
		s = thelutil.Bytes2Hex(tx.From())
	}
	if rec.Len() != 0 {
		r = strings.Trim(rec.Str(), "\x00")
	} else {
		if core.MessageCreatesContract(tx) {
			r = thelutil.Bytes2Hex(core.AddressFromMessage(tx))
		} else {
			r = thelutil.Bytes2Hex(tx.To())
		}
	}
	ptx.Sender = s
	ptx.Address = r
...
func (gui *Gui) setWalletValue(amount, unconfirmedFunds *big.Int) {
	...
	if unconfirmedFunds != nil {
		...
		val := thelutil.CurrencyToString(new(big.Int).Abs(thelutil.BigCopy(unconfirmedFunds)))
		str = fmt.Sprintf("%v (%s %v)", thelutil.CurrencyToString(amount), pos, val)
	} else {
		str = fmt.Sprintf("%v", thelutil.CurrencyToString(amount))
	}
...
func (gui *Gui) update() {
	...
	gui.win.Root().Call("setWalletValue", fmt.Sprintf("%v", thelutil.CurrencyToString(state.GetAccount(gui.address()).Balance())))
...
func (gui *Gui) privateKey() string {
	return thelutil.Bytes2Hex(gui.eth.KeyManager().PrivateKey())
}
