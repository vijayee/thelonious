...
func (self *UiLib) LookupName(addr string) string {
	var (
		...
		lookup  = nameReg.Storage(thelutil.Hex2Bytes(addr))
	)
...
func (self *UiLib) LookupAddress(name string) string {
	var (
		nameReg = self.World().Config().Get("NameReg")
		lookup  = nameReg.Storage(thelutil.RightPadBytes([]byte(name), 32))
	)

	if lookup.Len() != 0 {
		return thelutil.Bytes2Hex(lookup.Bytes())
	}

	return ""
}

func (self *UiLib) PastPeers() *thelutil.List {
	return thelutil.NewList([]string{})
	//return thelutil.NewList(eth.PastPeers())
}

func (self *UiLib) ImportTx(rlpTx string) {
	tx := types.NewTransactionFromBytes(thelutil.Hex2Bytes(rlpTx))
...
func (self *UiLib) StartDbWithContractAndData(contractHash, data string) {
	...
	object := self.eth.ChainManager().State().GetStateObject(thelutil.Hex2Bytes(contractHash))
	if len(object.Code) > 0 {
		dbWindow.SetCode("0x" + thelutil.Bytes2Hex(object.Code))
	}
...
func (self *UiLib) Compile(code string) (string, error) {
	bcode, err := thelutil.Compile(code, false)
	if err != nil {
		return err.Error(), err
	}

	return thelutil.Bytes2Hex(bcode), err
}
...
func (self *UiLib) AddLocalTransaction(to, data, gas, gasPrice, value string) int {
	return self.miner.AddLocalTx(&miner.LocalTx{
		To:       thelutil.Hex2Bytes(to),
		Data:     thelutil.Hex2Bytes(data),
		Gas:      gas,
		GasPrice: gasPrice,
		Value:    value,
	}) - 1
...
func (self *UiLib) SetGasPrice(price string) {
	self.miner.MinAcceptedGasPrice = thelutil.Big(price)
}
...
func (self *UiLib) ToHex(data string) string {
	return "0x" + thelutil.Bytes2Hex([]byte(data))
}
...
func (self *UiLib) ToAscii(data string) string {
	...
	return string(thelutil.Hex2Bytes(data[start:]))
}
...
func (self *UiLib) Messages(id int) *thelutil.List {
...
func mapToTxParams(object map[string]interface{}) map[string]string {
	...
	for _, str := range data {
		if thelutil.IsHex(str) {
			...
			if len(str) != 64 {
				str = thelutil.LeftPadString(str, 64)
			}
		} else {
			str = thelutil.Bytes2Hex(thelutil.LeftPadBytes(thelutil.Big(str).Bytes(), 32))
		}
